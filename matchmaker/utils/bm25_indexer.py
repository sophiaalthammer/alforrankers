...
import sys
import argparse
from matchmaker.utils.elastic_utils import ElasticSearch as elasticsearch_util
import ast
import json
...
def get_docs(index_name, file_path, application_in_training, type):
    ...
    with open(file_path) as fp:
        for line in fp:
            ...
            id_ = line_strip[0].strip()
            contents = line_strip[1].strip()
            doc = {
                "_index": index_name,
                "_id": id_,
...
def get_mapping(type):
    if type == "para":
        return json.loads(open("./config/lexical_retrieval/paragraph_level_mapping_bm25.json").read())
    else:
        return json.loads(open("./config/lexical_retrieval/whole_doc_mapping_bm25.json").read())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--replace_index', dest='replace_index', type=lambda x: bool(strtobool(x)),
                        help="remove current index!")
    ...
    #index_name = 'clefip-para-claims-together'
