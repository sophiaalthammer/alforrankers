...
def load_ranking(path,qrels=None,only_judged=False):
    with open(path,'r') as f:
        ...
        for l in f:
            try:
                ...
                if len(l) == 6: # original trec format
                    ...
                    pid = l[2].strip()
                    rank = int(l[3])
                if only_judged and qrels is not None:
                    if qid not in qrels or pid not in qrels[qid]:
                        continue
