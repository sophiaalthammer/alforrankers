...
def prepare_fixed_experiment_folder(base_path, run_name):
...
def prepare_experiment(args, config):
    ...
    elif args.run_name_fixed is not None:
        run_folder = prepare_fixed_experiment_folder(config["expirement_base_path"], args.run_name_fixed)
    else:
        raise ValueError("run_name or run_name_fixed must be set")
...
def print_hello(config:dict,run_folder:str,train_mode,show_settings=["Model","Train Batch Size","Train Learning Rate","Max Doc. Length","Init"]):
    ...
    embedding_tech = config["token_embedder_type"] if not config["token_embedder_type"].startswith("bert") else config.get("bert_pretrained_model","")
    settings = {

        "Model": config.get("model","") +" ("+embedding_tech+")",
...
def gather_tensors(*tt: Tensor, local_rank):
    tt = [torch.cat(_gather_tensor(t, local_rank=local_rank)) for t in tt]
    return tt


def split_tensor_dict(td: Dict[str, Tensor], cache_chunk_size):
    keys = list(td.keys())
    chunked_tensors = [td[k].split(cache_chunk_size) for k in keys]
