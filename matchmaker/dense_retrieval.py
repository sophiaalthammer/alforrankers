...
if __name__ == "__main__":
    ...
    parser_rn.add_argument('--run-name', action='store', dest='run_name', help='run name, used for the run folder (no spaces, special characters), gets timestamp added automatically')
    parser_rn.add_argument('--run-name-fixed', action='store', dest='run_name_fixed', help='fixed run name, used for the run folder (no spaces, special characters)')
        ...
        #
            ...
            for f in range(0,len(glob.glob(os.path.join(encode_folder,"token_reps_*")))):
                storage.append(np.memmap(os.path.join(encode_folder,"token_reps_"+str(f)+".npy"), dtype=numpy.dtype(encode_config["token_dtype"]),
                                                   mode="r", shape=(token_base_size,vector_dimensions))[:storage_filled_to_index[f]])
                    ...
                    for sample_i, query_id in enumerate(batch_orig["seq_id"]):  # operate on cpu memory
                        ...
                        if model_config["model"] == "ColBERT":
                            ...
                            for t, s_idx in enumerate(current_ids):
                                if seq_ids[s_idx] in unique_ids:
                                    continue
                                unique_ids.add(seq_ids[s_idx])
