...
class Claps(nn.Module):
    def __init__(self, config):
        super(Claps, self).__init__()
        self.tau = config['claps_tau']#args.tau
        self.pos_eps = config['claps_eps_pos'] #args.pos_eps
        self.neg_eps = config['claps_eps_neg'] #args.neg_eps

        self.projection = nn.Sequential(nn.Linear(config['claps_hidden_size'], config['claps_hidden_size']),
                                        nn.ReLU())
