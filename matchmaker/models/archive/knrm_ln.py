...
class KNRM_LN(Model):
    ...
    def __init__(self,
        ...
        self.length_norm_factor = nn.Parameter(torch.full([n_kernels],0.5,requires_grad=True).cuda())
        #self.length_norm_factor = nn.Parameter(torch.full([1],0.5,requires_grad=True).cuda())
        #self.length_norm_factor2 = nn.Parameter(torch.full([n_kernels],0.5,requires_grad=True).cuda())
    ...
    def forward(self, query: Dict[str, torch.Tensor], document: Dict[str, torch.Tensor],
        ...
        #l_norm = self.length_norm_factor2 * (1 - self.length_norm_factor + self.length_norm_factor * (document_length.float().unsqueeze(1) / 70))

        #l_norm = l_norm.unsqueeze(1)

        l_norm = (1 - self.length_norm_factor + self.length_norm_factor * (document_length.float().unsqueeze(1)))
        l_norm = l_norm.unsqueeze(1)
        #l_norm = l_norm.view(-1,1,1)
        length_normalized = log_per_kernel_query_masked / l_norm #(l_norm + log_per_kernel_query_masked)

        per_kernel = torch.sum(length_normalized, 1) 
