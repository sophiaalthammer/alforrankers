...
class Conv_KNRM_Same_Gram(Model):
    '''
    This version only matches 1-1, 2-2, 3-3 grams without cross matching
    ...
    def __init__(self,
        ...
        # *3 because we concat the 3 conv match sums together before the dense layer
        self.dense = nn.Linear(n_kernels * 3, 1, bias=False) 
    ...
    def forward(self, query: Dict[str, torch.Tensor], document: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        all_grams = torch.cat([sim_q1_d1,sim_q2_d2,sim_q3_d3],1)
    ...
    def forward_matrix_kernel_pooling(self, query_tensor, document_tensor, query_by_doc_mask, query_pad_oov_mask):
        ...
        log_per_kernel_query = torch.log1p(torch.clamp(per_kernel_query, min=1e-10)) # * 0.01
