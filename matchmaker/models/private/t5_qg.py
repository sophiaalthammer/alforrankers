#
# based on https://github.com/seanie12/CLAPS/blob/main/src/qg/models.py
#
...
class T5AdvContrastiveQG(nn.Module):
    ...
    def from_config(config):
        cfg = T5Config()
        cfg.t5_model            = config["bert_pretrained_model"]
        cfg.tau                 = config["t5_tau"]
        cfg.neg_eps             = config["t5_pos_eps"]
        cfg.pos_eps             = config["t5_neg_eps"]
        cfg.hidden_size         = config["t5_hidden_size"]
        cfg.adv                 = config.get("t5_adversarial", False)
        cfg.trainable           = config.get('bert_trainable', True)
        return T5AdvContrastiveQG(cfg)

    def __init__(self, cfg) -> None:
        super().__init__(cfg)
        self.tau = cfg.tau
        self.pos_eps = cfg.pos_eps
        self.neg_eps = cfg.neg_eps

        self.t5_model = T5ForConditionalGeneration.from_pretrained(cfg.t5_model)
        self.projection = nn.Sequential(nn.Linear(cfg.hidden_size, cfg.hidden_size),
                                        nn.ReLU())
    ...
    def forward(self, input_ids, attention_mask,
                ...
                lm_labels, adv=False): # query: Dict[str, torch.LongTensor],document: Dict[str, torch.LongTensor],
                #use_fp16:bool = True,
