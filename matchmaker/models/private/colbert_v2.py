...
class ColBERT_v2(PreTrainedModel):
    """
    ...
    def forward(self,
        ...
        with torch.cuda.amp.autocast(enabled=use_fp16):
            ...
            score_matrix = torch.bmm(query_vecs, document_vecs.transpose(2,1))
            score_matrix[~(query["attention_mask"].bool()).unsqueeze(-1).expand(-1,-1,score_matrix.shape[-1])] = 0
            score_per_term = score_matrix.mean(1)

            score_per_term[~(document["attention_mask"].bool())] = - 1000

            #score = score_per_term.max(-1).values

            if score_per_term.shape[1] < self.top_k_chunks:
                score_per_term = nn.functional.pad(score_per_term,(0, self.top_k_chunks - score_per_term.shape[1]),value=-1000)

            score = torch.sort(score_per_term,descending=True,dim=-1).values
            score[score <= -999] = 0

            score = (score[:,:self.top_k_chunks] * self.top_k_scoring).mean(dim=1)
...
##
    ...
    def get_param_stats(self):
        return "ColBERT: top_k_scoring:" + str(self.top_k_scoring.data)
