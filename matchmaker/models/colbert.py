...
class ColBERT(PreTrainedModel):
    ...
    def get_output_dim(self):
        if self.use_compressor:
            return self.compressor.out_features
        elif hasattr(self.bert_model.config, "dim"):
            return self.bert_model.config.dim
        else:
    ...
    def forward_aggregation(self,query_vecs:torch.Tensor,query_mask, document_vecs, document_mask):
        score = torch.bmm(query_vecs.expand((document_vecs.shape[0],-1,-1)), document_vecs.transpose(2,1))
        score[~(document_mask.bool()).unsqueeze(1).expand(-1,score.shape[1],-1)] = - 1000
        
        score = score.max(-1).values
        
        score[~(query_mask.expand((document_vecs.shape[0],-1)).bool())] = 0
