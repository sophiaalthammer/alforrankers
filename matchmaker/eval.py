...
def evaluate_model(model, config, _logger, cuda_device, eval_tsv, use_cache=False, 
                   output_secondary_output: bool = False,status=None):
    ...
    try:
        ...
        if status != None: 
            base_status = str(status.renderable)
        with torch.no_grad():
            if status != None: status.update(base_status + " - [bold green] Starting evaluation..",refresh=True)
            pair_count = 0
            if train_sparsity:
                stopwords=0
                total_words=0
            ...
            for batch_orig in input_loader:
                ...
                i+=1
                if status != None and i % 10 == 0: status.update(base_status + " - [bold green]  Evaluation Progress ... Batch No.: " + str(i), refresh=True)
...
def validate_model(val_name, model, config, validation_config, run_folder, _logger, cuda_device, 
                   ...
                   output_secondary_output: bool = False,is_distributed=False,status=None):
    ...
    validation_results,qa_validation_results,qa_validation_results_stats,pair_count,secondary_output = evaluate_model(model,config,_logger,cuda_device,validation_config["tsv"],use_cache,output_secondary_output,status)
...
#
def test_model(model, config,test_config, run_folder, _logger, cuda_device, test_name,candidate_set = None, candidate_set_n = None,output_secondary_output=False,is_distributed=False,status=None):

    test_results,qa_validation_results,qa_validation_results_stats,_,secondary_output = evaluate_model(model,config,_logger,cuda_device,test_config["tsv"],use_cache=False,output_secondary_output=output_secondary_output,status=status)
