...
class IdSequenceDatasetReader(DatasetReader):
    ...
    def __init__(self,
                 ...
                 split_document_window_size:int = -1,
                 split_document_overlap:int = -1,
        ...
        self.split_document_window_size = split_document_window_size
        self.split_document_overlap = split_document_overlap
        ...
        with open(cached_path(file_path), "r", encoding="utf8") as data_file:
            for line in self.shard_iterable(data_file):
                ...
                if self.split_document:
                    splits = seq_text.split()[:4000]
                    windows = list(range(0,math.ceil(len(splits)/self.split_document_window_size)))
                    if len(windows) > 1:
                        windows = windows[:-1]
                    for i in windows:
                        #print(seq_id,len(splits),i*self.split_document_window_size,(i+1)*self.split_document_window_size)
                        yield self.text_to_instance(seq_id, " ".join(splits[i*self.split_document_window_size:(i+1)*self.split_document_window_size]))
                else:
    ...
    def text_to_instance(self, seq_id:str, seq_text:str) -> Instance:
        ...
        if self.token_type == "huggingface":
            ...
            if not isinstance(seq_tokenized, list):
                if 'token_type_ids' in seq_tokenized.keys():
                    seq_tokenized.pop('token_type_ids')
            seq_field = CustomTransformerTextField(**seq_tokenized)
