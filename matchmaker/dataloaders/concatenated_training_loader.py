...
class ConcatenatedTrainingDatasetReader(DatasetReader):
    ...
    def text_to_instance(self, query_sequence: str, doc_pos_sequence: str, doc_neg_sequence: str,pos_score,neg_score,qa_spans_pos) -> Instance:  # type: ignore
        ...
        doc_pos_tokenized = self._tokenizer.tokenize(query_sequence,doc_pos_sequence,self.max_query_length + self.max_doc_length)
        if doc_pos_tokenized.get('token_type_ids') is not None:
            doc_pos_tokenized.pop('token_type_ids')

        doc_neg_tokenized = self._tokenizer.tokenize(query_sequence,doc_neg_sequence,self.max_query_length + self.max_doc_length)
        if doc_neg_tokenized.get('token_type_ids') is not None:
            doc_neg_tokenized.pop('token_type_ids')

        ret_instance = {
                "doc_pos_tokens": CustomTransformerTextField(**doc_pos_tokenized),
                "doc_neg_tokens": CustomTransformerTextField(**doc_neg_tokenized)}
