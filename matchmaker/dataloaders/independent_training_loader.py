...
class IndependentTrainingDatasetReader(DatasetReader):
    ...
    def __init__(self,
        ...
        if type(tokenizer) == FastTransformerTokenizer or type(tokenizer) == T5Tokenizer:
        ...
        self.add_text_to_batch = add_text_to_batch
    ...
    def text_to_instance(self, query_sequence: str, doc_pos_sequence: str, doc_neg_sequence: str, pos_title, neg_title,
        ...
        if self.token_type == "huggingface":
            ...
            if self.query_augment_mask_number > -1:
                ...
                query_tokenized["attention_mask"] = torch.nn.functional.pad(query_tokenized["attention_mask"],
                                                                            (0, self.query_augment_mask_number),
                                                                            value=1)
            if not isinstance(query_tokenized, list):
                if 'token_type_ids' in query_tokenized.keys():
                    query_tokenized.pop('token_type_ids')
            query_field = CustomTransformerTextField(**query_tokenized)
        ...
        if self.token_type == "huggingface":
            ...
            if not isinstance(doc_pos_tokenized, list):
                if 'token_type_ids' in doc_pos_tokenized.keys():
                    doc_pos_tokenized.pop('token_type_ids')
            doc_pos_field = CustomTransformerTextField(**doc_pos_tokenized)
        ...
        if self.token_type == "huggingface":
            ...
            if not isinstance(doc_neg_tokenized, list):
                if 'token_type_ids' in doc_neg_tokenized.keys():
                    doc_neg_tokenized.pop('token_type_ids')
            doc_neg_field = CustomTransformerTextField(**doc_neg_tokenized)
        else:
