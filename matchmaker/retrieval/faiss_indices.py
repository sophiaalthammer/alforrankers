...
class FaissBaseIndexer(BaseNNIndexer):
    ...
    def __init__(self,config, dimensions):
        super(FaissBaseIndexer, self).__init__(config, dimensions)
    ...
    def index(self, ids:List[numpy.ndarray], data_chunks:List[numpy.ndarray]):
        # single add needed for multi-gpu index (sharded), and hnsw
...
class FaissIdIndexer(FaissBaseIndexer):
    ...
    def __init__(self,config,dims):
        super(FaissIdIndexer, self).__init__(config,dims)

        if self.use_gpu:

            console.log("[FaissIdIndexer]","Index on GPU")
            cpu_index = faiss.IndexIDMap(faiss.IndexFlatIP(self.token_dim))
            ...
            if self.use_fp16:
                self.faiss_index = faiss.IndexIDMap(faiss.IndexScalarQuantizer(self.token_dim,faiss.ScalarQuantizer.QT_fp16,faiss.METRIC_INNER_PRODUCT))
            else:
                self.faiss_index = faiss.IndexIDMap(faiss.IndexFlatIP(self.token_dim))

    def index(self, ids:List[numpy.ndarray], data_chunks:List[numpy.ndarray]):
        if not self.use_gpu:
            for i, chunk in enumerate(data_chunks):
                console.log("[FaissIdIndexer]","Add chunk:",i,chunk.shape,ids[i].shape)
...
class FaissHNSWIndexer(FaissBaseIndexer):
    ...
    def __init__(self,config, dimensions):
        super(FaissHNSWIndexer, self).__init__(config, dimensions)

        self.use_gpu = False # HNSW does not support GPUs

        if self.use_fp16:
            console.log("[FaissHNSWIndexer]","Index with fp16")
            self.faiss_index = faiss.IndexHNSWSQ(self.token_dim,faiss.ScalarQuantizer.QT_fp16,
                                                config["faiss_hnsw_graph_neighbors"],faiss.METRIC_INNER_PRODUCT)

        else:
            console.log("[FaissHNSWIndexer]","Index with fp32")
            self.faiss_index = faiss.IndexHNSWFlat(self.token_dim,config["faiss_hnsw_graph_neighbors"],faiss.METRIC_INNER_PRODUCT)
...
class FaissIVFIndexer(FaissBaseIndexer):
    ...
    def __init__(self, config, dimensions):
        super(FaissIVFIndexer, self).__init__(config, dimensions)
        ...
        if self.use_fp16:
            ...
            self.faiss_index = faiss.IndexIVFScalarQuantizer(
                faiss.IndexScalarQuantizer(self.token_dim, faiss.ScalarQuantizer.QT_fp16, faiss.METRIC_INNER_PRODUCT),
            ...
            self.faiss_index = faiss.IndexIVFFlat(faiss.IndexFlatIP(self.token_dim), self.token_dim,
                                                  self.faiss_ivf_list_count, faiss.METRIC_INNER_PRODUCT)
    ...
    def prepare(self, data_chunks: List[numpy.ndarray], subsample=-1):
        ...
        console.log("[FaissIVFIndexer]", "Train", len(all_vecs), "entries", self.faiss_ivf_list_count,
                    "lists (avg-goal/list: ", len(all_vecs) // self.faiss_ivf_list_count, ")")
    ...
    def search_single(self, query_vec, top_n):
        ...
        #
        centroid_dist, centroid_ids = self.faiss_index.search(query_vec, self.list_n_probe)
...
class FaissShardedOnDiskIdIndexer():
    ...
    def __init__(self,config, dimensions ,run_folder):
        super(FaissShardedOnDiskIdIndexer, self).__init__()
        self.token_dim=dimensions
...
class FaissDynamicIndexer():
    ...
    def __init__(self,config,dimensions):
        ...
        #faiss.IndexIVFScalarQuantizer(faiss.IndexHNSWSQ(self.token_dim, faiss.ScalarQuantizer.QT_fp16, 12, faiss.METRIC_INNER_PRODUCT),self.token_dim,config["faiss_ivf_lists"],faiss.ScalarQuantizer.QT_fp16,faiss.METRIC_INNER_PRODUCT)
        #self.faiss_index = faiss.IndexIVFFlat(self.token_dim,config["faiss_ivf_lists"],faiss.ScalarQuantizer.QT_fp16,faiss.METRIC_INNER_PRODUCT)
        
        #self.faiss_index = faiss.IndexIDMap(faiss.index_factory(self.token_dim,config["faiss_index_options"],faiss.METRIC_INNER_PRODUCT))
        self.list_n_probe = 1
        self.token_dim = dimensions
        self.faiss_ivf_list_count = config["faiss_ivf_list_count"]
    ...
    def prepare(self, data_chunks:List[numpy.ndarray], subsample=-1):
        ...
        if subsample > -1:
            ...
            train_vecs = numpy.concatenate(data_chunks).astype(numpy.float32)
